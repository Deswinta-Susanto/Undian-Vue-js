<template>
    <CRow>
      <CCol :xs="12">
        <CCard class="mb-4">
          <CCardHeader>
            <strong>Update Kandidat Hadiah Utama</strong>
          </CCardHeader>
          <CCardBody>
  ...
  export default {
    ...
    methods: {
        ...
        axios
          .get(`http://127.0.0.1:8000/api/kandidat-hadiah-utama/get/${id}`, {
      ...
      submitForm() {
        ...
        axios
          .put(`http://127.0.0.1:8000/api/kandidat-hadiah-utama/update/${this.id}`, payload, {
            headers: { Authorization: `Bearer ${token}` }
          })
