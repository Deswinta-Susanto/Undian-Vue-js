<template>
    ...
    <button @click="$router.push({ name: 'AddKandidatUtama' })"
...
export default {
    ...
    try {
        ...
        const response = await axios.post("http://127.0.0.1:8000/api/kandidat-hadiah-utama/import", formData, {
            headers: {
...
async fetchKandidats() {
  ...
  try {
    ...
    const response = await axios.get("http://127.0.0.1:8000/api/kandidat-hadiah-utama/get", {
      params: {
    ...
    if (response.status === 200) {
      ...
      console.error("Unexpected response while fetching kandidats:", response.data);
    }
  } catch (error) {
