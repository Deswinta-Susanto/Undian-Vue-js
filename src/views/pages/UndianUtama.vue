...
const startRandom = async () => {
    ...
    stopRandom(); // Menghentikan interval sebelumnya jika ada
    randomNumbers.value = []; // Mengosongkan data randomNumbers

    try {
        const response = await axios.get(
            "http://127.0.0.1:8000/api/kandidat-hadiah-utama/get",
            {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
...
const submitWinners = async () => {
    // Bentuk array riwayat sesuai format
    const payload = {
        riwayat: randomNumbers.value.map((set) => ({
...
const stopRandom = () => {
    if (interval) {
        ...
        // Finalisasi pemenang untuk ditampilkan
        randomNumbers.value = randomNumbers.value.map((set) => ({
            ...set,
            numbers: set.numbers,
            winner: set.winner,
        }));

        // Kirim data pemenang ke API
        submitWinners();
    }
};
...
const generateRandomFromNIPP = (nipp) => {
    const numbers = nipp
        .split("") // Ubah NIPP menjadi array karakter
        .map((char) => (isNaN(parseInt(char)) ? 0 : parseInt(char))); // Ubah karakter menjadi angka (0 jika bukan angka)

    // Batasi hanya hingga 8 digit
    if (numbers.length > 8) {
        return numbers.slice(0, 8);
    }

    // Tambahkan angka 0 di akhir hingga panjang 8 digit
    while (numbers.length < 8) {
        numbers.push(0);
    }

    return numbers; // Kembalikan tepat 8 angka
};
