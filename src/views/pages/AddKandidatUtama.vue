<template>
    <CRow>
      <CCol :xs="12">
        <CCard class="mb-4">
          <CCardHeader>
            <strong>Tambah Kandidat Hadiah Utama</strong>
          </CCardHeader>
          <CCardBody>
  ...
  export default {
    ...
    methods: {
      submitForm() {
        ...
        axios
          .post("http://127.0.0.1:8000/api/kandidat-hadiah-utama/store", payload, {
            headers: { Authorization: `Bearer ${token}` }
          })
          .then(() => this.$router.push("/pages/ListKandidatUtama"))
